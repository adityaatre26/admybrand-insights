...
export const DataTable = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'completed': return 'text-foreground-muted bg-muted/10';
      default: return 'text-foreground-muted bg-muted/10';
    }
  };
                ...
                {isLoading ? (
                  ...
                  paginatedData.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(row.status)}`}>
                          {row.status}
                        </span>
                      </td>
                    </motion.tr>
            ...
            <div className="flex items-center space-x-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <Button
                  ...
                  className={`w-8 h-8 p-0 ${currentPage === page ? '' : 'glass-hover'}`}
                >
                  {page}
                </Button>
              ))}
